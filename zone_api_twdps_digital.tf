...
provider "aws" {
  alias  = "subdomain_api_twdps_digital"
  region = "us-east-2"
  assume_role {
...
module "subdomain_api_twdps_digital" {
  ...
  providers = {
    aws = aws.subdomain_api_twdps_digital
  }

  zones = {
    "api.${local.domain_twdps_digital}" = {
      tags = {
  ...
  records = [
    {
      ...
      records         = module.subdomain_api_twdps_digital.route53_zone_name_servers["api.${local.domain_twdps_digital}"]
    }
  ]

  depends_on = [module.subdomain_api_twdps_digital]
}
